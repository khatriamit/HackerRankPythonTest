...
he has decided to take a couple of old songs and make dubstep remixes from them. Let's assume that a song consists of 
some number of words (that don't contain WUB). To make the dubstep remix of this song, Polycarpus inserts a certain 
number of words "WUB" before the first word of the song (the number may be zero), after the last word (the number may be zero), 
...
def song_decoder1(song):
    my_string = re.sub("WUB", " ", song)
    return " ".join((my_string.strip(" ")).split())
...
res = "WUBWUBWUBWEWUBAREWUBWUBTHEWUBCHAMPIONSWUBMYWUBFRIENDWUB"
out = song_decoder2(res)
print(out)
